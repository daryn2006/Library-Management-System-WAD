...
public class LibraryServlet extends BaseLmsServlet {
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
            throws ServletException, IOException {
        String editId = req.getParameter("editId");
        if (editId != null && isAdmin(req)) {
            Library library = dataService.findLibraryById(Integer.parseInt(editId));
            req.setAttribute("editLibrary", library);
        }
        List<Library> libraries = dataService.getLibraries();
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        if ("create".equals(action)) {
            dataService.createLibrary(name, address);
        } else if ("update".equals(action) && id != null) {
            dataService.updateLibrary(Integer.parseInt(id), name, address);
        } else if ("delete".equals(action) && id != null) {
            dataService.deleteLibrary(Integer.parseInt(id));
        }
